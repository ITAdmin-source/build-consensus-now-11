...
export const OptimizedVotingInterface: React.FC<OptimizedVotingInterfaceProps> = React.memo(({
  ...
  if (!statement) {
    return <div className="space-y-6 animate-fade-in">
        <div className="text-center p-6 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg border border-green-200">
          ...
          <Button onClick={onViewResults} className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 transition-all duration-300">
            צפה בתוצאות
          </Button>
        </div>
  ...
  const getSwipeIndicatorText = () => {
    switch (swipeState.direction) {
      case 'right':
        return buttonLabels.support;
      case 'left':
        return buttonLabels.oppose;
      case 'down':
        return buttonLabels.unsure;
      default:
        return '';
    }
  };
  ...
  return <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Card ref={cardRef} className={`poll-card transition-all duration-300 cursor-grab active:cursor-grabbing select-none ${pendingVote ? 'scale-[0.98] opacity-80' : 'scale-100 opacity-100'} ${swipeState.isDragging ? `${getSwipeIndicatorColor()} border-2` : ''} animate-scale-in`} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onMouseDown={handleMouseStart} onMouseMove={swipeState.isDragging ? handleMouseMove : undefined} onMouseUp={handleMouseEnd} onMouseLeave={handleMouseEnd}>
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <Button onClick={() => handleOptimisticVote('support')} disabled={!!pendingVote} className={`vote-button bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white py-6 transition-all duration-300 ${pendingVote === 'support' ? 'ring-4 ring-green-300 scale-105' : ''}`} size="lg">
                <ThumbsUp className="h-6 w-6 ml-2" />
                <span className="hebrew-text text-lg">{buttonLabels.support}</span>
              </Button>
              
              <Button onClick={() => handleOptimisticVote('unsure')} disabled={!!pendingVote} className={`vote-button bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white py-6 transition-all duration-300 ${pendingVote === 'unsure' ? 'ring-4 ring-yellow-300 scale-105' : ''}`} size="lg">
                <HelpCircle className="h-6 w-6 ml-2" />
                <span className="hebrew-text text-lg">{buttonLabels.unsure}</span>
              </Button>
              
              <Button onClick={() => handleOptimisticVote('oppose')} disabled={!!pendingVote} className={`vote-button bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white py-6 transition-all duration-300 ${pendingVote === 'oppose' ? 'ring-4 ring-red-300 scale-105' : ''}`} size="lg">
